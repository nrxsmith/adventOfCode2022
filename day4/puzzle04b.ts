...
const isPairOverlapping = (pair1: number[], pair2: number[]) => {
    if (pair1[1] >= pair2[0] && pair1[1] <= pair2[1]) return true
    if (pair2[1] >= pair1[0] && pair2[1] <= pair1[1]) return true
    return false
}
...
file.on('line', (line) => {
    ...
    if (isPairOverlapping(numPairs[0], numPairs[1])) totalContainedPairs++
})
